...
import { Star, Film, Tv, Play, Sparkles, Flame, Shield, Clock, Compass, Zap, Crown, Smile } from "lucide-react"
...
const FALLBACK_SERIES: VODItem[] = [
  { id: 201, title: "The Boys", type: "Série", rating: 8.7, poster: null, year: "2024", overview: "" },
  { id: 202, title: "House of the Dragon", type: "Série", rating: 8.4, poster: null, year: "2024", overview: "" },
  { id: 203, title: "Shōgun", type: "Série", rating: 8.7, poster: null, year: "2024", overview: "" },
  { id: 204, title: "Fallout", type: "Série", rating: 8.3, poster: null, year: "2024", overview: "" },
  { id: 205, title: "The Penguin", type: "Série", rating: 8.5, poster: null, year: "2024", overview: "" },
  { id: 206, title: "Severance", type: "Série", rating: 8.7, poster: null, year: "2024", overview: "" },
]
...
export function TrendingVOD() {
  ...
  const movieItems = vidéos.length > 0 ? vidéos : FALLBACK_MOVIES
  const doubledMovies = [...movieItems, ...movieItems]

  const seriesItems = series.length > 0 ? series : FALLBACK_SERIES
  const doubledSeries = [...seriesItems, ...seriesItems]

  const renderVODCard = (item: VODItem, idx: number, prefix: string) => (
    <div
      key={`${prefix}-${item.id || idx}-${idx}`}
    ...
    <div className="space-y-12 w-full">
      {loading ? (
        ...
        <div className="space-y-12">
          {/* Section 1: Films Tendances */}
          <div className="space-y-4">
            <h3 className="text-lg sm:text-xl font-black text-white uppercase tracking-wider flex items-center gap-2 px-1">
              ...
              <div className="bg-[#0c101d] rounded-3xl p-6 sm:p-8 border border-white/5 overflow-hidden">
                <style>{`
                  ...
                  .movies-track {
                    ...
                    gap: 1.5rem;
                    width: max-content;
                    animation: scrollMovies 45s linear infinite;
                ...
                `}</style>
                <div className="movies-track pb-2">
                  {doubledMovies.map((item, idx) => renderVODCard(item, idx, "movie"))}
                </div>
              </div>
            </div>
          </div>

          {/* Section 2: Séries Tendances */}
          <div className="space-y-4">
            <h3 className="text-lg sm:text-xl font-black text-white uppercase tracking-wider flex items-center gap-2 px-1">
              <Tv className="w-5 h-5 text-[#a855f7]" />
              Séries Tendances
            </h3>
              ...
              <div className="bg-[#0c101d] rounded-3xl p-6 sm:p-8 border border-white/5 overflow-hidden">
                <style>{`
                  ...
                  .series-track {
                    ...
                    gap: 1.5rem;
                    width: max-content;
                    animation: scrollSeries 45s linear infinite;
                    will-change: transform;
                    backface-visibility: hidden;
                  }
                  .series-track:hover {
                ...
                `}</style>
                <div className="series-track pb-2">
                  {doubledSeries.map((item, idx) => renderVODCard(item, idx, "series"))}
                </div>
              </div>
            </div>
          </div>
